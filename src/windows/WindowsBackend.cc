...
WindowsBackend::~WindowsBackend() {
  ...
  mRunning = false;
  QueueUserAPC([](__in ULONG_PTR) {}, (HANDLE)mHandle,
               (ULONG_PTR)this);
}
...
public:
  ...
  void processEvents(DWORD errorCode) {
    ...
    case ERROR_ACCESS_DENIED: {
      // This can happen if the watched directory is deleted. Check if that is
      // the case, and if so emit a delete event. Otherwise, fall through to
      // default error case.
  ...
  void processEvent(PFILE_NOTIFY_INFORMATION info) {
    ...
    case FILE_ACTION_RENAMED_NEW_NAME: {
      ...
      if (GetFileAttributesExW(utf8ToUtf16(path).data(), GetFileExInfoStandard,
                               &data)) {
